...
* `int sprintf(char *buf, const char *format, ...);` - массив, в котором следует сформировать результирующую строку, необходимо передать функции в первый параметр., остальные параметры как у `printf`, форматная строка анализируется по тем же правилам, что и `printf`. Обеспечить буфер нужного размера, обязанность вызывающего.
> Если применяете директиву `%s`, размер строки стоит ограничить указав точность - `%.20s`, гарантирует что больше 20 символов не будет вставлено.

> В `C99` стандарте есть функция `snprintf` которая принимает размер буфрера.

Пример программы переводящей число в его текстовое представление:
```
char str[32];
int n;
/*...*/
