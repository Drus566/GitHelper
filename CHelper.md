...
```
Первый параметр - исходный файл, второй - измененный. Пример сравнения файлов:
```
5c5
<	printf("Hello, world!\n");
---
> 	printf("Hello, world\n");
```
`5c5` в начале выдачи означает, что строка 5 превратилась в строку 5 (не изменили количество строк). Далле `diff` печатает фрагмент оригинального текста, предваряя каждую его строку символом `<`, затем выдает разделитель `---` и печатает фрагмент в его измененной версии, используя символ `>`.

Если бы мы не редактировали существующую строку, а вставили новые, `diff` отразил бы этот факт буквой `a` (от слова `append`). Удалению строк соответствует буква `d` (delete).

Можно изменить формат вывода `diff` при указании флага `-y`, она будет печатать в две колонки, слева - текст оригинального файла, справа - текст измененного. Колонки отделяются друг от друга стобцом символов, обозначающих, что произошло с данной строкой. Если строка не изменилась, печатает пробел. Если удалена то `<`. Если добавлена то `>`. Для изменившейся строки - `|`.

Также можно указать ширину программы - `-W 80`, установит ширину в 80 символов. Например:
```
diff -y -W 80 hello.c hello2.c
```

От программы `diff` чаще требуется представить изменения в виде, предназначенном для другой программы -  `patch`.

`patch` предназначена для того, чтобы к копии исходного файла применить изменения, ранее обнаруженные в помощью `diff`.

Форматы описания изменений, специально предназначенные для такого применения, сделаны так, что в большинстве случаев программа `patch` может в полностью автоматическом режиме внести требуемые изменения даже в копию исходного файла, "не совсем" тождественную той, которая использовалась при запуске `diff`: скажем, если два программиста редактировали один и тот же файл, но в разных его местах (например один в начале, другой в конце), и один отправил свои изменения другому в виде выдачи `diff`, то в большинстве случаев второй программист сможет получить в своей копии исходных текстов изменения, внесенные первым программистом, несмотря на то, что в его копии файла нумерация строк уже совершенно другая.

Достигается это включением в выдачу `diff` так называемого контекста - нескольких (чаще всего трех) неизменившихся строк, предшествующих модифицированному фрагменту, и стольких же непосредственно следующих за ним. 

Программа `patch` использует контекст, чтобы привязаться к изменившийся	нумерации строк исходного файла.

Основных форматов выдачи `diff`, подразумевавших контекст, существует два: 
* контекстный (флаг `-c`)
* унифицированный (флаг `-u`). В современных услових используется именно он.

Также `diff` позволяет сравнивать директории с помощью флага `-r`, и `-N`, который предписывает `diff` рассматривать отсутствующие файлы и пустые. Например: `diff -Nur old new`.

> Дата 1970-01-01 означает отсутствие даты

Опытные программисты часто, особенно при работе на языке Си, рекомендуют к флагам `-u`, `-r`, `-N` добавить `-p`, который заставит `diff` указывать для каждого изменения, к какой функции языка Си это изменение относится.

Итоговый набор флагов проще всего запомнить так: `-Nurp`.

Итак например мы получаем патч (`old-new.diff`): `diff -Nurp old new > old-new.diff`

В большинстве случаев программе патч следует указать  параметр `-p1`. Параметр `p` в общем случае предписывает игнорировать заданное количество элементов имен файлов, тоесть программа `patch`, получив флаг `-p` с неким числом `N`, удаляет из имен файлов `N` первых символов `/` вместе со всеми буквами между ними. 

В подавляющем большинстве случаев программу `patch` запускают именно так - с параметром `-p1`, как правило, патч-файл получают путем сравнения двух директорий точно так же, как это сделали мы, а имена этих директорий при наложении патча следует проигнорировать.

Пример патчинга файла: `patch -p1 < ../old-new.patch`

Пример патчинга в обратную сторону: `patch -p1 -R < ../old-new.patch`

> В конце строки в тексте нельзя оставлять назначащие (невидимые) пробелы.

### Системы контроля версий
База данных, в которой хранится вся история зарегистрированных изменений исходных текстов, называется репозиторием. Единожды внесенная в репозиторий информация остается в нем навсегда.

`git stash` - сохранение временно где-то все изменения, которые успели внести в рабочую директорию в индекс.
`git stash pop --index` - восстановление состояния рабочей директории и индекса.

Когда репозиторий создается как копия другого, в создаваемом репозитории его исходный репозиторий автоматически регистрируется в качестве удаленного под именем `origin`.

Манипулировать списком зарегистрированных удаленных репозиториев позволяет команда `git remote`. Например `git remote` выдаст список удаленных репозиториев. Более подробные результаты можно получить добавив `-v`, `git remote -v`.	

Добавить новый удаленный репозиторий - `get remote add <репозиторий>`

Удаление удаленного репозитория - `git remote rm`

Переименование удаленного репозитория - `git remote rename`

Для каждого удаленного репозитория `git`, когда у него есть такая возможность, создает локально ветки, соответствующие состоянию веток удаленного репозитория. Имена этих веток например такие: `remotes/origin/master`, `remotes/devserv/fixup21`. Первый вариант соответствует ветке `master` из удаленного репозитория `origin`. Второй ветке `fixup21` из репозитория `devserv`. 

Удаленные ветки не показываются командой `git branch` если об этом не попросить командой `git branch -a`.

Удаленные ветки отличаются от обычных тем, что в них нельзя ничего регистрировать, то есть делать в них `commit`. К удаленной ветке можно применить `git checkout`, чтобы посмотреть, как выглядит рабочая директория в той или иной ветке удаленного репозитория.

Узнать для каких веток заданы вышестоящие ветки - `git branch -vv`

`git fetch` - получение свежей информации из удаленных репозиториев.

`git merge` - скачивание изменений из удаленного репозитория.

Если удаленный репозиторий отслеживается, для получения и скачивания изменений из удаленного репозитория используется `git pull`.

Отправка изменений на удаленный репозиторий `git push boss_computer crazy:stable_version`. Отправит содержимое локальной ветки с именем `crazy` в удаленный репозиторий `boss_computer`, где это содержимое будет отражено в ветке, имеющей название `stable_version`. Если такой ветки там нет, она будет создана.

Разрешение принимать изменения в текущую (активную) ветку - `git config receive.denyCurrentBranch warn`. Теперь можно отсылать изменения в удаленную активную ветку. Но будет предупреждение, увидев его надо ввести `git reset --hard HEAD`, эта команда предписывает `git` привести файлы в рабочей директории в соответствие с состоянием `HEAD`, не обращая внимание любые несоответствия.
