...
### Уменьшить/Увеличить масштаб терминала 
```
Ctrl + -
Ctrl Shitft +
```
...
### Фильтры 
#### Выделяет строки, удовлетворяющие условиям
`grep`
#### Сортировка 
`sort`
#### Выделяет из каждой строки некую подстроку 
`cut`
#### Подсчитывает количество символов слов и строк 
`wc`
### Имитирование конца файла в терминале 
`Ctrl + D`
...
#### Убрать переменную из окружения
```
unset MYVAR
export MYVAR
```
#### Модифицированное окружение для отдельной команды
```
VAR=value command
```
...
### Протоколирование script
```
$ script my_protocol.txt
Script started, file is my_protocol.txt
$ ls
Ctrl + D - выйти из протоколирования
```
> Запись всех действий в `my_protocol.txt`

### Проверка xdm сервера в сети
`X -broadcast`
### Запуск оконного менеджера
`startx`
### Стирание(сдвиг вверх) текста терминала
`ctrl + win + l`
...
Ctrl + L
```
### Если зашли в root права, то выход из них
```
Ctrl + D
```
### Выход в свой терминал из удаленной машины
```
Ctrl + D
```
### Расширения в Linux
Расширения в файлах это обычный текст, файл может быть либо бинарник, либо текст.
### Удаление файла 
`rm <file>`

### Очищение консоли до курсора 
`Ctrl + U`

### Создать файл и добавить пару строчек в него 
```
# Добавление в конец
echo "/dist" >> .gitignore
# Перезапись
echo "/dist" > .gitignore
```
### Команда не отображается в `history`
Если команда написана с пробелом вначале

### Труба, по которой текут данные
`|`
Т.е., `history` выплюнет историю в трубу, по которой инфа потечет к другой проге

### Поиск текста
```
grep
history | grep gitignore
grep <что хотим искать> <в какой директории>
grep sleep ./*
greep -r sleep ./* # рекурсивный поиск
```
### Настройки bash
```
cat ~/.bashrc
```
### Поиск в истории с помощью горячих клавиш
`Ctrl + R`
> Чтобы пойти в поиск в глубину жмем - `Ctrl + R`

### Конкатенация вывод нескольких файлов 
```
cat hihi huhu
```
...
su - drug1 
```
### Добавление юзеру прав рута
```
usermod -aG sudo drug1
```
### Конфиг суперюзеров
```
sudo visudo
...
### Переход в начало консоли, конец строки
```
ctrl + A
ctrl + E
```
...
`dpkg -i <путь к файлу>`
Установка зависимостей `apt-get install -f`
