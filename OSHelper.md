...
### Многопоточное программирование в Unix

Создание и запуска потока:
```
int pthread_create(pthread_t* thr, pthread_attr_t* attr, void*(*start_routine)(void*), void* arg);
```
* `thr` - в какую переменную следует записать идентификатор нового потока.
* `attr` - специфические параметры работы нового потока
* `start_routine` - указывает на главную функцию потока.
...
Поток может дождаться завершения другого потока с помощью `pthread_join`:
```
int pthread_join(pthread_t th, void **result);
```
* `th` - идентификатор треда, завершение которого мы хотим подождать
* `result` - адрес указателя типа `void*`, в который следует записать результат работы потока.

Перевод потока в **отсоединенный режим** (чтобы он не занимал системные ресурсы):
```
int pthread_detach(pthread_t th);
```

Узнать собственный идентификатор:
```
pthread_t pthread_self();
```
Например перевод потоком самого себя в отсоединенный режим:
```
pthread_detach(pthread_self());
```

Завершение потоком досрочно другого потока:
```
int pthread_cancel(pthread_t th);
```
* Результатом работы потока `th` будет специальное значение `PTHREAD_CANCELED`.


### Семафоры и мьютексы
Мьютекс инициализатор:
```
...
int pthread_mutex_trylock(pthread_mutex_t *mutex); // неблокирующее закрытие мьютекса
```
...
> Чтобы уничтожить мьютекс, он должен быть открыт.

Инициализация семафора:
```
int sem_init(sem_t *sem, int pshared, unsigned int value);
```
* `sem` - адрес инициализируемого семафора
* `pshared` - указывает, будет ли семафор доступен для других процессов
...
Неблокирующий вариант операции `down`:
```
...
```
* Значение семафора возвращается через `sval`.

Удаление семафора:
```
int sem_destroy(sem_t *sem);
```
* Не должно быть ни одного треда(процесса) находящегося в состоянии ожидания.

### Разделяемые данные на диске
Файловые захваты, поддерживаемые на уровне ОС, не работают для файлов, находящихся на сетевых дисках.

Основные подходы к захвату доступа к файлу:
* С захватываемым файлом создается еще один файл, который показывает другим, что в настоящее время файл уже захвачен (не требует поддержки ОС)
* BSD locks - системный вызов `flock`
* POSIX lockf - функция `lockf`, работающая через системный вызов `fcntl`.

`int flock(int fd, int operation);`
* `operation`: 
1. `LOCK_SH` - разделяемый
2. `LOCK_EX` - эксклюзивный
3. `LOCK_UN` - убрать ранее установленный

`int lockf(int fd, int cmd, int len);`

### Ядро системы, взгляд за кулисы
Контекст выполнения процесса - значения всех регистров процессора на момент снятия процесса с исполнения. В памяти ядра для всего этого создается структура данных, которая представляет собой процесс как объект.
