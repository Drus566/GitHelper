...
%strlen sl 'my string'
```
#### Выделение из строки символа с заданным номером
```
%substr var 1 'abcd' 1
```
## Взаимодействие с ОС
Способ построения мультизадачности, при котором смена активной задачи происходит только в случае её окончания или запроса на операцию ввода-вывода, называется **пакетным режимом**, а ОС, реализующие этот режим, **пакетными ОС**. (самый эффективный режим с точки зрения использования мощности ЦП).
...
**Режимы работы ЦП**:
* привилегированный (режим ядра, kernal mode, supervisor mode)
* ограниченный (пользовательский режим, user mode)

Для реализации мультизадачной ОС **аппаратное обеспечение компьютера обязано поддерживать**:
* аппарат прерываний
* защиту памяти
* привилегированный и ограниченный режимы работы процессора
* таймер
...
```
### Алгоритм работы редактора связей
#### Флаги
* `-l` - указание имени библиотеки (отбрасывается префикс `lib` и суффикс `.a`) `libgreet.a => greet`. Также при указании этого флага, редактор связей пытается найти соответствующую библиотеку в `/lib`, `/usr/lib` и т.п.)
* `-L` - означает, что следует сначала попробовать найти библиотеку в текущей директории, а затем в системных.
> Объектные модули следует указывать раньше, чем библиотеки. И еще библиотеки не должны перекрестно зависеть друг от друга.
